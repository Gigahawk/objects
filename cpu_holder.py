...
tab_mount_hole_width_tol = 0.1
tab_mount_hole_thickness_tol = 0.1
...
with BuildPart() as holder_parent:
    ...
    with BuildSketch(parent_center_loc) as marker_sketch:
        ...
        with Locations(marker_loc):
            Triangle(
                a=clamshell_wall_thickness / 2,
                b=clamshell_wall_thickness / 2,
                C=90,
    ...
    with BuildLine() as tab_path:
        _base_line = FilletPolyline(
            [
                (0, tab_mount_hole_depth, 0),
                (0, -(tab_catch_depth + tab_thickness / 2), 0),
                (
                    0,
                    -(tab_catch_depth + tab_thickness / 2),
                    tab_hole_height_rem + clamshell_thickness,
                ),
            ],
    ...
    tab_top_edge = (
        holder_parent.edges(Select.LAST)
        .filter_by(Axis.X)
...
holder_parent_main = holder_parent.part.solids().sort_by(Axis.Y)[-1]
holder_parent_tab = holder_parent.part.solids().sort_by(Axis.Y)[0]
...
results = {
    "asm_print": Compound([holder_parent_main, holder_child.part]),
    "asm_rotated": Compound([holder_parent.part, holder_child_rotated]),
    "tab": holder_parent_tab,
}
