...
def build(A, B, C, D, E):
    ...
    with BuildPart() as part:
        ...
        chamfer(cyl_edges, length=cylinder_chamfer, length2=cylinder_chamfer2)
        # Cylinder cut
        cylinder_cut_plane = (
            part.faces().sort_by(Axis.X)[0].offset(-cylinder_cut_offset)
        )
        with BuildSketch(cylinder_cut_plane):
            ...
            Rectangle(
                100, wall_thickness, align=(Align.MIN, Align.CENTER), mode=Mode.SUBTRACT
            )
        ...
        with BuildSketch(main_workplane):
            ...
            with Locations((A, B)):
                Rectangle(
                    A - wall_straight_length,
                    B - wall_straight_length,
                    align=Align.MAX,
                    mode=Mode.INTERSECT,
                )
            offset(amount=pocket_offset, mode=Mode.REPLACE)
