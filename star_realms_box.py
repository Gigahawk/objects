...
tray_cards_length = (
    wall_thickness + trade_thickness + wall_thickness + trade_thickness + wall_thickness
)
...
tray_width = wall_thickness + card_width + wall_thickness + card_width + wall_thickness
...
cover_clearance = 1.5
cover_inner_height = (
    card_height + tray_bottom_thickness - cover_lip_thickness + cover_clearance
)
...
retaining_nub_neg_base_rad = (
    retaining_nub_pos_dia / 2 / sin(radians(retaining_nub_chamfer_angle))
)
retaining_nub_neg_height = min(
    retaining_nub_pos_dia / 2 + retaining_nub_tol,
    retaining_nub_neg_base_rad * tan(radians(retaining_nub_chamfer_angle)),
)
...
retaining_nub_chamfer_depth2 = retaining_nub_chamfer_depth / tan(
    radians(retaining_nub_chamfer_angle)
)
...
with BuildPart() as tray:
    Box(tray_length, tray_width, tray_height, align=(Align.MIN, Align.MIN, Align.MAX))
    ...
    with BuildSketch() as cards_layout:
        with Locations((wall_thickness, wall_thickness)):
            with GridLocations(
                x_spacing=trade_thickness + wall_thickness,
                y_spacing=0,
                y_count=1,
                x_count=2,
                align=Align.MIN,
        ...
        with Locations((wall_thickness, 2 * wall_thickness + card_width)):
            with GridLocations(
                x_spacing=starting_thickness + wall_thickness + starting_extra_space,
                y_spacing=0,
                y_count=1,
                x_count=starting_decks,
                align=Align.MIN,
            ):
                Rectangle(starting_thickness, card_width, align=Align.MIN)
        with Locations(
            (tray_cards_length - wall_thickness, tray_width - wall_thickness)
        ):
    ...
    with BuildSketch() as challenge_cards_layout:
        with Locations(
            (tray_cards_length + challenge_cards_thickness / 2, tray_width / 2)
        ):
            Rectangle(challenge_cards_thickness, challenge_card_width)
    extrude(amount=-tray_challenge_cards_depth, mode=Mode.SUBTRACT)
    challenge_cards_surface = (
        tray.faces(select=Select.LAST).filter_by(Axis.Z).sort_by(Axis.Z)[0]
    )
    ...
    cover_lip_top_surface = (
        tray.faces(select=Select.LAST)
        .filter_by(Axis.Z)
        .sort_by(Axis.Z, reverse=True)[0]
    )
...
_logo_outline = scale(
    mirror(make_face(import_svg("res/star_realms_logo.svg")), about=Plane.XZ),
    by=logo_xy_scale,
)

with BuildPart() as cover:
    with BuildSketch(
        cover_lip_top_surface, mode=Mode.PRIVATE
    ) as cover_walls_base_sketch:
        _cover_project_sketch = make_face(
            project(bottom_surface.edges(), mode=Mode.PRIVATE)
        )
    with BuildSketch(
        cover_lip_top_surface, mode=Mode.PRIVATE
    ) as cover_walls_outer_sketch:
        offset(
            _cover_project_sketch,
            cover_wall_thickness + cover_tol / 2,
            kind=Kind.INTERSECTION,
        )
    with BuildSketch(
        cover_lip_top_surface, mode=Mode.PRIVATE
    ) as cover_walls_inner_sketch:
    ...
    with BuildSketch(cover_inner) as cover_top_sketch:
        ...
        add(
            cover_walls_outer_sketch.sketch.moved(
                Location(-cover_walls_outer_sketch.sketch.center())
            )
        )
    extrude(amount=cover_wall_thickness)
    cover_top, cover_top_inner = (
        cover.faces().filter_by(Axis.Z).sort_by(Axis.Z, reverse=True)[:2]
    )
    chamfer(cover_top.edges(), length=cover_top_chamfer)
    if cover_top_chamfer - cover_wall_thickness > 0:
        chamfer(
            cover_top_inner.edges(), length=cover_top_chamfer - cover_wall_thickness
        )
    ...
    nub_neg = extrude(
        amount=retaining_nub_neg_height,
        taper=retaining_nub_neg_taper,
        mode=Mode.SUBTRACT,
    )
    ...
    with Locations(
        Location(
            (tray_length / 2, tray_width / 2, -tray_height + cover_lip_thickness),
            (90, 0, 0),
        ),
        Location(
            (tray_length / 2, tray_width / 2, -tray_height + cover_lip_thickness),
        ...
        Cylinder(
            cover_finger_dia / 2, max(tray_length, tray_width) * 2, mode=Mode.SUBTRACT
        )

    finger_inner_edges = (
        cover.edges(select=Select.LAST)
    ...
    chamfer(
        finger_inner_edges,
        length=retaining_nub_chamfer_depth,
        length2=retaining_nub_chamfer_depth2,
    )

    with BuildSketch(_plane_width_center) as grip_sketch:
        with Locations(
            (
                -cover_tol / 2 - cover_wall_thickness,
                -tray_height + cover_grip_height_pos,
            )
        ):
            Trapezoid(
                cover_grip_height,
                cover_grip_depth,
                left_side_angle=cover_grip_angle,
                rotation=-90,
    ...
    with BuildSketch(cover_top, mode=Mode.PRIVATE) as _logo_sketch:
        offset(_logo_outline, logo_tol, kind=Kind.INTERSECTION)
    ...
    RigidJoint(
        label="logo_face", joint_location=Location(logo_face.bounding_box().center())
    )
...
_logo = Solid(_logo)
_logo = scale(_logo, by=(logo_xy_scale, logo_xy_scale, logo_z_scale))
