...
with BuildPart() as mount:
    ...
    guide_right_face = (
        mount.faces(Select.LAST).filter_by(Axis.X).sort_by(Axis.X, reverse=True)[0]
    )
    ...
    with BuildSketch() as tab_sketch:
        with Locations(tab_edge.start_point()):
            Rectangle(tab_support_width, tab_gap, align=(Align.MIN, Align.MAX))
            with Locations((0, -tab_gap)):
                Rectangle(
                    tab_support_width + tab_length,
                    tab_thickness,
    ...
    guide_edge = (
        mount.edges()
        .filter_by(Axis.Z)
        .sort_by_distance(mount_face.center() + (guide_outer_width / 2, 0, 0))
    )[0]
    tab_support_edge = (
        mount.edges()
        .filter_by(Axis.Z)
        .sort_by_distance(
            mount_face.center() - (guide_outer_width / 2 + tab_support_width, 0, 0)
        )
    )[0]
    ...
    with BuildSketch(mount_face.offset(support_gap)) as guide_support_sketch:
        with Locations(
            ...
            Rectangle(
                guide_support_hole_width, guide_support_hole_width, mode=Mode.SUBTRACT
            )
    extrude(amount=guide_thickness - support_gap - tab_gap)
    guide_support_chamfer_edge = (
        mount.edges(Select.LAST)
        .filter_by(Axis.Z)
        .sort_by(Axis.X, reverse=True)[0:2]
        .sort_by(Axis.Y, reverse=True)[0]
    )
