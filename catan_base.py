...
with BuildPart() as base:
    # Frame base
    with BuildSketch():
        RegularPolygon(radius=outer_flat_dist / 2, side_count=6, major_radius=False)
    extrude(amount=base_height)

    # Tapered section meeting art
    with BuildSketch(base.faces().sort_by(Axis.Z)[-1]):
        RegularPolygon(radius=taper_flat_dist / 2, side_count=6, major_radius=False)
    ...
    with BuildSketch(Plane.ZY) as slot:
        with Locations(Location(magnet_slot_start, magnet_slot_angle - 90)):
            ...
            with Locations((0, -magnet_slot_dia / 2)):
                Rectangle(
                    magnet_slot_bump_length,
                    magnet_slot_bump_height,
        ...
        filter_axis = Axis(
            ...
            direction=(
                0,
                -cos(radians(magnet_slot_angle)),
                sin(radians(magnet_slot_angle)),
            ),
        )
        points = slot.vertices().filter_by_position(
            axis=filter_axis,
...
with BuildPart() as plug:
    with BuildSketch(Plane.ZY) as plug_sketch:
        with Locations(Location(magnet_slot_start, magnet_slot_angle - 90)):
            Rectangle(
                magnet_slot_depth,
                magnet_slot_dia - plug_clearance,
                align=(Align.MIN, Align.CENTER),
            )
            Circle(radius=(magnet_slot_dia + plug_clearance) / 2, mode=Mode.SUBTRACT)
            with Locations((0, -magnet_slot_dia / 2)):
                Rectangle(
